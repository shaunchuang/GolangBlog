      ...
      <div key={article.id} className="card mb-4 shadow-sm h-100">
        ...
        <div className="card-body d-flex flex-column">
          ...
          <div className="mt-auto">
            ...
            <Link href={`/article-detail/${article.id}`} className="btn btn-primary">
              閱讀更多
            </Link>
            
            {/* 僅對編輯者和管理員顯示編輯按鈕 */}
            {state.auth.user && ['admin', 'editor'].includes(state.auth.user.role) && (
              <Link href={`/admin/articles/edit/${article.id}`} className="btn btn-outline-secondary ms-2">
                編輯
              </Link>
            )}
          </div>
    ...
    <div className="container py-4">
      ...
      {state.auth.user && ['admin', 'editor'].includes(state.auth.user.role) && (
        <div className="text-center mt-4">
          <Link href="/admin/articles/new" className="btn btn-success">
